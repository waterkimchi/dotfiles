...
clone_repo() {
	if check_nvim_version "${REQUIRED_NVIM_VERSION}"; then
		execute "git" "clone" "-b" "master" "${CLONE_ATTR[@]}" "$1" "${DEST_DIR}"
...
info "This script will install waterkimchi/dotfiles to:"
echo "${DEST_DIR}"
...
if [[ "${USE_SSH}" -eq "1" ]]; then
	clone_repo "git@github.com:waterkimchi/dotfiles.git"
else
	clone_repo "https://github.com/waterkimchi/dotfiles.git"
fi
...
- Project Homepage:
    ${tty_underline}https://github.com/waterkimchi/dotfiles${tty_reset}
EOS
